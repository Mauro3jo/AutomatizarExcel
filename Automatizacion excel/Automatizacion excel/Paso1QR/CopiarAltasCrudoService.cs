...
{
    public class CopiarAltasCrudoService
    {
        public void CopiarAltas(string rutaCRM, string rutaCrudo, Action<string, int>? reportarProgreso = null)
        {
            ...
            try
            {
                ...
                var hojaCRM = wbCRM.Sheets["ALTAS"] as Excel.Worksheet;
                var hojaCrudo = wbCrudo.Sheets["Altas"] as Excel.Worksheet;

                if (hojaCRM == null || hojaCrudo == null)
                    throw new Exception("No se encontraron las hojas ALTAS o Altas.");

                int lastRowCRM = hojaCRM.Cells.SpecialCells(Excel.XlCellType.xlCellTypeLastCell).Row;
                int colHasta = 68; // BP = 68
                int filasCopiadas = 0;
                ...
                wbCrudo.Save();
                reportarProgreso?.Invoke($"✅ {filasCopiadas} filas copiadas correctamente.", 100);
            }
            catch (Exception ex)
            {
                reportarProgreso?.Invoke("❌ Error al copiar: " + ex.Message, 0);
                throw;
            }
            finally
            {
